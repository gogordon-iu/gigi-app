...
// Used for secure local token generation, verification, and API key encryption.
...
// Configurable Admin Passcode & Salt Utilities
let _inMemoryPasscode: string | null = null;
let _inMemorySalt: string | null = null;

export function getAdminPasscode(): string {
  if (_inMemoryPasscode) return _inMemoryPasscode;

  try {
    if (typeof localStorage !== 'undefined') {
      const stored = localStorage.getItem('gigi_admin_passcode');
      if (stored && stored.trim()) return stored.trim();
    }
  } catch {}

  // Fallback to environment variables if provided
  let envPasscode: string | undefined;
  if (typeof process !== 'undefined' && process.env?.REACT_APP_ADMIN_PASSCODE) {
    envPasscode = process.env.REACT_APP_ADMIN_PASSCODE;
  } else {
    try {
      const meta = new Function('return import.meta')() as any;
      envPasscode = meta?.env?.VITE_ADMIN_PASSCODE;
    } catch {}
  }
  if (envPasscode) return envPasscode.trim();
...
export function setAdminPasscode(newPasscode: string): void {
  ...
  _inMemoryPasscode = trimmed;
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('gigi_admin_passcode', trimmed);
    }
  } catch {}
}

export function getTokenSalt(): string {
  if (_inMemorySalt) return _inMemorySalt;

  try {
    if (typeof localStorage !== 'undefined') {
      const stored = localStorage.getItem('gigi_token_salt');
      if (stored && stored.trim()) return stored.trim();
    }
  } catch {}

  let envSalt: string | undefined;
  if (typeof process !== 'undefined' && process.env?.REACT_APP_TOKEN_SALT) {
    envSalt = process.env.REACT_APP_TOKEN_SALT;
  } else {
    try {
      const meta = new Function('return import.meta')() as any;
      envSalt = meta?.env?.VITE_TOKEN_SALT;
    } catch {}
  }
  if (envSalt) return envSalt.trim();

  return 'gigi-token-salt-2026';
}

export function setTokenSalt(newSalt: string): void {
  const trimmed = newSalt.trim();
  _inMemorySalt = trimmed;
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('gigi_token_salt', trimmed);
    }
  } catch {}
}
